...
par(mar = c(1, 1, 1, 1))
plot(QUGA, col = "darkgreen", main = "Garry oak distribution")
...
par(mar = c(1, 1, 1, 1))
plot(QUGA, col = "darkgreen", main = "Garry oak sensitivity to climate change")
plot(s.map, add = T, 
     smallplot = c(.7, .71, .3, .7), 
     axis.args = list(at = c(s.map@data@min, s.map@data@max), labels = c("low", "high")))
```
