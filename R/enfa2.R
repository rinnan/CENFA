...
setGeneric("enfa2", function(x, s.dat, ...){
  standardGeneric("enfa2")})
...
setMethod("enfa2",
          ...
          function(x, s.dat, filename = "", progress = FALSE, parallel = FALSE, n = 1, ...){
            ...
            if (canProcessInMemory(x.crop)){
              ...
              #Sm <- sweep(S, 2, mar)
              DpSm <- apply(S, 2, function(x) x * p)
              Rs <- crossprod(S, DpSm)/ (p.sum - 1)
            } else {
              ...
              if (progress) cat("Calculating species covariance matrix...\n")
              #Sm <- parScale(x.mask, center = F, scale = F, parallel = parallel, n = n, progress = F)
              Rs <- parCov(x = x.mask, w = s.dat, parallel = parallel, n = n, progress = progress)
            }
...
setMethod("enfa2",
          ...
          function(x, s.dat, field, fun = "last", filename = "", progress = FALSE, parallel = FALSE, n = 1, ...){
            ...
            mod <- enfa2(x = x, s.dat = s.dat.ras, filename = filename, progress = progress, parallel = parallel, n = n, ...)
            mod@call <- call
            return(mod)
          }
)
...
setMethod("enfa2",
          ...
          function(x, s.dat, scale = TRUE, filename = "", progress = FALSE, parallel = FALSE, n = 1, ...){
            ...
            mod <- enfa2(x = x, s.dat = s.dat, filename = filename, progress = progress, parallel = parallel, n = n, ...)
            mod@call <- call
            return(mod)
          }
)
